        ...
        to_time ||= from_time + distance.seconds
          while distance > 0
            if distance < 1.minute
              output["segundos"] = distance.to_i
              distance = 0
            elsif distance < 1.hour
              output["minutos"], distance = distance.divmod(1.minute)
            elsif distance < 1.day
              output["horas"], distance = distance.divmod(1.hour)
            elsif distance < 28.days
              output["días"], distance = distance.divmod(1.day)
      ...
      def display_time_in_words(hash, include_seconds = false, options = {})
        ...
        hash.delete(:seconds) if !include_seconds && hash[:minutes]
          ...
          time_measurements = ["years",
                              "meses","semanas","días",
                               "horas", "minutos", "segundos"].delete_if do |key|
          ...
          output.to_sentence(options).gsub!("and", "y")
      end
